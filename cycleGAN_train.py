...
def main():
    ...
    parser.add_argument("--model-name", type=str, default="cyclegan_v3")
    ...
    for epoch in tqdm(range(trained_epoch, args.num_epoch), desc='Epoch', total=args.num_epoch, initial=trained_epoch):
        ...
        for x_F, x_Q, _ in tqdm(train_dataloader, desc='Step'):
            ...
            e_adv_loss.append(G_adv_loss.item())
            ...
            D_adv_loss_F = adv_loss(x_F,torch.ones(x_F.size()).to(args.device)) + adv_loss(D_F(x_QF),torch.zeros(out_D_F.size()).to(args.device))
            D_adv_loss_Q = adv_loss(x_Q,torch.ones(x_Q.size()).to(args.device)) + adv_loss(D_Q(x_FQ),torch.zeros(out_D_Q.size()).to(args.device))
        ...
        print(f'🔥🔥🔥🔥 ADV-LOSS-e-{epoch}: {np.mean(e_adv_loss)}')
